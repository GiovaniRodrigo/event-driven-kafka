...
export async function runScenarioBNormal(ctx: BenchmarkContext): Promise<Record<string, any>> {
  console.log('\n===============================================================');
  console.log(' SCENARIO B: NORMAL LOAD BENCHMARK (35 req/s Sustained)');
  console.log('===============================================================');
  ...
  console.log('>> Executing normal load: 35 req/s for 30 seconds (~1,050 orders)...');
  const loadResult = await loadGen.runConstantLoad({
    ratePerSec: 35,
    durationSeconds: 30,
    userIdPrefix: 'user_normal',
  });
  loadGen.close();
  ...
  const result = {
    scenario: 'scenario-b-normal-load',
    timestamp: new Date().toISOString(),
    environment: ctx.getEnvironmentInfo(),
    workload: {
      targetRatePerSec: 35,
      durationSeconds: 30,
      totalOrdersTarget: 1050,
    },
    httpMetrics: {
  ...
  ctx.saveBenchmarkResult('normal-load', result);
